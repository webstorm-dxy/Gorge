...
pub struct ClassInfo {
    ...
    pub field_start_type_count: crate::compile_context::symbol::FrozenTypeCount,
    /// 含继承的实例字段各值类型总数
    pub field_type_count_total: crate::compile_context::symbol::FrozenTypeCount,
...
pub struct ConstructorInfo {
    ...
    pub parameters: Vec<ParameterId>,
    pub is_native: bool,
    /// 是否为注入器构造方法（带 `injector` 修饰符）
    pub is_injector: bool,
    /// 在类的注入器构造方法中的局部编号（仅 is_injector 为 true 时有效）
    pub injector_local_id: Option<usize>,
    pub body_scope_id: Option<ScopeId>,
    pub span: Span,
}
...
/// 代码块上下文类型
///
/// 影响变量查找策略和注入器字段/实例字段读写的权限校验。
///
/// 权限矩阵（对齐 C# `BlockContext.cs`）：
/// | 上下文          | 注入器字段读 | 注入器字段写 | 实例字段 | this |
/// |----------------|:---------:|:---------:|:------:|:----:|
/// | `Constant`     | -         | -         | -      | -    |
/// | `StaticMethod` | -         | -         | -      | -    |
/// | `Injector`     | 可读      | 可写      | 不可   | 无   |
/// | `FieldInjecting`| 可读     | 不可写     | 可读写  | 有   |
/// | `Instance`     | 不可      | 不可      | 可读写  | 有   |
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BlockContext {
    /// 常量求值上下文，只能求编译时常量
    Constant,
    /// 静态方法内部，不能操作实例字段和 this
    StaticMethod,
    /// 注入器上下文，只能操作注入器字段，不能操作实例字段
    Injector,
    /// 字段初始化器上下文（实例字段初始化表达式求值）
    FieldInitializer,
    /// 注入中上下文（字段初始化器带 @Inject 注解时的特殊上下文，注入器字段只读不可写）
    FieldInjecting,
    /// 实例方法/实例构造方法内部
    Instance,
}
...
impl SymbolTable {
    ...
    /// 检查指定作用域中是否已存在同名符号（仅本级，不查父作用域）
    ///
    /// B-2: 用于重复声明检测。方法重载（`SymbolEntry::Method`）和构造方法重载
    /// 允许同名不同签名，在 declare_method/declare_constructor 中独立处理。
    pub fn has_symbol_in_scope(&self, scope_id: ScopeId, name: &str) -> bool {
        self.scopes.get(scope_id.0).symbols.contains_key(name)
    }
    ...
    pub fn declare_constructor(
        ...
        is_native: bool,
        is_injector: bool,
        injector_local_id: Option<usize>,
        span: Span,
    ) -> ConstructorId {
