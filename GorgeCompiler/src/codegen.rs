...
impl<'a> CodeGenerator<'a> {
    ...
    fn resolve_instance_method(&self, class_name: &str, method: &str, arg_types: &[TypeInfo]) -> Result<Option<(usize, ValueType)>, ()> {
        ...
        loop {
            ...
            for (i, &method_id) in class_info.methods.iter().enumerate() {
                ...
                let global_id = class_info.method_start_id + i;
            ...
            Expression::Lambda { parameters, body, span } => {
                ...
                let return_type = match body {
                    ...
                    LambdaBody::Block(ref stmts) => {
                        ...
                        ValueType::Object // 块体默认返回 Object
                    }
                };
                ...
                let free_vars_count = free_vars.len(); // I4: 0 表示静态委托
                self.delegate_impls.push(DelegateImpl {
                    ...
                    is_static: free_vars_count == 0, // I-B: 无捕获变量则为静态委托
                });
    ...
    fn generate_assignment(
        ...
        match target {
            ...
            AssignmentTarget::InjectorField { object, field, span: _ } => {
    ...
    fn generate_method_call(
        ...
        if delegate_idx.is_none() {
            ...
            // 变量.方法()：若已知变量的类，解析实例方法编号与返回类型（含重载）
            if let Expression::Identifier(var_name, _) = receiver {
                if let Some(class_name) = self.var_class.get(var_name).cloned() {
                    ...
                    if let Ok(Some((method_idx, ret_vt))) = self.resolve_instance_method(&class_name, method, &arg_types) {
                        self.param_counters.reset();
                        for arg in arguments { let arg_op = self.generate_expression(arg); self.emit_set_param(arg_op, span); }
                        let recv_op = self.generate_expression(receiver);
    ...
    fn generate_new(
        ...
        let ctor_id = target.as_ref().and_then(|name| {
            let scope = self.symbol_table.global_scope;
            self.symbol_table.lookup_class(scope, name).map(|cid| {
                let ci = self.symbol_table.classes.get(cid.0);
                let start = ci.constructor_start_id;
                ci.constructors.iter().enumerate()
                    .find(|(_, ctor_cid)| self.symbol_table.constructors.get(ctor_cid.0).parameters.len() == arguments.len())
                    .map(|(i, _)| start + i)
                    .unwrap_or(start)
            })
        }).unwrap_or(0);
