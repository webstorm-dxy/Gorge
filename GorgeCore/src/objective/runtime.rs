...
use std::sync::Arc;
use crate::objective::class::{GorgeClass, RuntimeClass};
use crate::objective::declaration::{EnumDef};
use crate::objective::interface::GorgeInterface;
...
impl GorgeRuntime {
    ...
    /// 按全名获取接口
    pub fn get_interface(&self, full_name: &str) -> Option<&Arc<GorgeInterface>> {
        self.interfaces.get(full_name)
    }

    /// 按全名获取枚举
    pub fn get_enum(&self, full_name: &str) -> Option<&Arc<EnumDef>> {
...
mod tests {
    ...
    /// get_interface / get_enum 公开查找
    #[test]
    fn test_a4_get_interface_enum() {
        let mut runtime = GorgeRuntime::new();
        runtime.register_interface(crate::objective::interface::GorgeInterface {
            name: "IShape".into(),
            full_name: "IShape".into(),
            methods: vec![],
            super_interfaces: vec![],
        });
        runtime.register_enum(EnumDef {
            name: "Color".into(),
            full_name: "Color".into(),
            values: vec![],
        });
        assert!(runtime.get_interface("IShape").is_some());
        assert!(runtime.get_interface("NoSuchIface").is_none());
        assert!(runtime.get_enum("Color").is_some());
        assert!(runtime.get_enum("NoSuchEnum").is_none());
    }

    /// class_names 遍历迭代器
    #[test]
    fn test_a4_class_names() {
        let mut runtime = GorgeRuntime::new();
        let decl_a = make_decl("Apple", None);
        let decl_b = make_decl("Banana", None);
        runtime.register_class(RuntimeClass::new(decl_a, None));
        runtime.register_class(RuntimeClass::new(decl_b, None));

        let names: Vec<&String> = runtime.class_names().collect();
        assert_eq!(names.len(), 2);
        assert!(names.iter().any(|n| n.as_str() == "Apple"));
        assert!(names.iter().any(|n| n.as_str() == "Banana"));
    }
}
