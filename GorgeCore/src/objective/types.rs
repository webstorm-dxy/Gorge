...
impl TypeCount {
    ...
    /// 逐项取最大值（对齐 C# TypeCount.Max）
    pub fn max(&mut self, other: &TypeCount) {
        self.int_count = self.int_count.max(other.int_count);
        self.float_count = self.float_count.max(other.float_count);
        self.bool_count = self.bool_count.max(other.bool_count);
        self.string_count = self.string_count.max(other.string_count);
        self.object_count = self.object_count.max(other.object_count);
    }

    /// 按基本类型将对应计数器 +1，并返回加之前的旧值作为索引
    ///
    /// 对齐 C# FixedFieldValuePool.Count(BasicType)（C# FixedFieldValuePool.cs:93-163），
    /// 用于字段分配时获取去重索引。
    pub fn count(&mut self, basic_type: BasicType) -> usize {
        let old = match basic_type {
            BasicType::Int | BasicType::Enum => {
                let v = self.int_count;
                self.int_count += 1;
                v
            }
            BasicType::Float => {
                let v = self.float_count;
                self.float_count += 1;
                v
            }
            BasicType::Bool => {
                let v = self.bool_count;
                self.bool_count += 1;
                v
            }
            BasicType::String => {
                let v = self.string_count;
                self.string_count += 1;
                v
            }
            BasicType::Null => 0,
            _ => {
                let v = self.object_count;
                self.object_count += 1;
                v
            }
        };
...
mod tests {
    ...
    // ==================== A-4 TypeCount::max / count 测试 ====================

    #[test]
    fn test_a4_type_count_max() {
        let mut a = TypeCount { int_count: 3, float_count: 1, ..TypeCount::zero() };
        let b = TypeCount { int_count: 5, float_count: 2, bool_count: 1, ..TypeCount::zero() };
        a.max(&b);
        assert_eq!(a.int_count, 5);
        assert_eq!(a.float_count, 2);
        assert_eq!(a.bool_count, 1);
    }

    #[test]
    fn test_a4_type_count_count() {
        let mut tc = TypeCount::zero();
        assert_eq!(tc.count(BasicType::Int), 0);
        assert_eq!(tc.count(BasicType::Int), 1);
        assert_eq!(tc.count(BasicType::Float), 0);
        assert_eq!(tc.count(BasicType::Object), 0);
        assert_eq!(tc.int_count, 2);
        assert_eq!(tc.float_count, 1);
        assert_eq!(tc.object_count, 1);
        // Enum 映射到 Int
        assert_eq!(tc.count(BasicType::Enum), 2);
        assert_eq!(tc.int_count, 3);
    }
}
