...
use crate::objective::declaration::{MethodAnnotation, AnnotationValue};
...
pub struct DelegateImpl {
    ...
    /// 捕获变量的值类型列表（1b），与 captured_var_names 一一对应。
    /// 用于运行时按类型分组将捕获值填入参数池。
    pub captured_var_types: Vec<ValueType>,
...
fn serialize_compiled_class(class: &CompiledClass, buf: &mut Vec<u8>) -> BytecodeResult<()> {
    ...
    for delegate in &class.delegate_impls {
        ...
        // V5: 捕获变量值类型序列化
        buf.extend_from_slice(&(delegate.captured_var_types.len() as u16).to_le_bytes());
        for ct in &delegate.captured_var_types {
            buf.push(value_type_to_u8(*ct));
        }
    ...
    // 方法注解（S3）
    serialize_method_annotations_map(&class.method_annotations, buf);
    // 构造方法注解（S3）
    serialize_method_annotations_map(&class.constructor_annotations, buf);

    Ok(())
}

/// 序列化方法注解映射表（S3）
fn serialize_method_annotations_map(annotations: &HashMap<usize, Vec<MethodAnnotation>>, buf: &mut Vec<u8>) {
    buf.extend_from_slice(&(annotations.len() as u16).to_le_bytes());
    for (method_id, anns) in annotations {
        buf.extend_from_slice(&(*method_id as u32).to_le_bytes());
        buf.extend_from_slice(&(anns.len() as u16).to_le_bytes());
        for ann in anns {
            let nb = ann.name.as_bytes();
            buf.extend_from_slice(&(nb.len() as u16).to_le_bytes());
            buf.extend_from_slice(nb);
            buf.extend_from_slice(&(ann.parameters.len() as u16).to_le_bytes());
            for (pn, pv) in &ann.parameters {
                let pb = pn.as_bytes();
                buf.extend_from_slice(&(pb.len() as u16).to_le_bytes());
                buf.extend_from_slice(pb);
                serialize_ann_value(pv, buf);
            }
        }
    }
}
...
/// 序列化注解值（S3）
fn serialize_ann_value(value: &AnnotationValue, buf: &mut Vec<u8>) {
    match value {
        AnnotationValue::Int(v) => { buf.push(0); buf.extend_from_slice(&v.to_le_bytes()); }
        AnnotationValue::Float(v) => { buf.push(1); buf.extend_from_slice(&v.to_le_bytes()); }
        AnnotationValue::Bool(v) => { buf.push(2); buf.push(if *v { 1 } else { 0 }); }
        AnnotationValue::String(v) => {
            buf.push(3); let vb = v.as_bytes(); buf.extend_from_slice(&(vb.len() as u16).to_le_bytes()); buf.extend_from_slice(vb);
        }
        AnnotationValue::Delegate(v) => { buf.push(4); buf.extend_from_slice(&(*v as u32).to_le_bytes()); }
    }
}
...
fn deserialize_compiled_class(data: &[u8], mut pos: usize, _version: u16) -> BytecodeResult<(CompiledClass, usize)> {
    ...
    for _ in 0..delegate_count {
        ...
        // V5: 读取捕获变量值类型
        let captured_var_types: Vec<ValueType> = if _version >= 5 {
            if pos + 2 > data.len() { return Err("读取委托捕获变量类型计数越界".into()); }
            let cvt_count = u16::from_le_bytes([data[pos], data[pos + 1]]) as usize;
            pos += 2;
            let mut cvt = Vec::with_capacity(cvt_count);
            for _ in 0..cvt_count {
                if pos >= data.len() { return Err("读取委托捕获变量类型越界".into()); }
                cvt.push(u8_to_value_type(data[pos]));
                pos += 1;
            }
            cvt
        } else {
        ...
        let cv_types_count = captured_var_types.len();
        let is_static = captured_var_types.is_empty();
        delegate_impls.push(DelegateImpl {
            ...
            captured_var_names,
            captured_var_types,
            outer_value_count: cv_types_count,
            is_static,
        });
    }
    ...
    // 方法注解（S3）—— 仅 V4 及以上格式有此段
    let mut method_annotations = HashMap::new();
    if _version >= 4 {
        if pos + 2 <= data.len() {
            method_annotations = deserialize_method_annotations_map(data, &mut pos)?;
        }
    }
    // 构造方法注解（S3）
    let mut constructor_annotations = HashMap::new();
    if _version >= 4 {
        if pos + 2 <= data.len() && pos < data.len() {
            constructor_annotations = deserialize_method_annotations_map(data, &mut pos)?;
        }
    }
...
/// 反序列化方法注解映射表（S3）
fn deserialize_method_annotations_map(data: &[u8], pos: &mut usize) -> BytecodeResult<HashMap<usize, Vec<MethodAnnotation>>> {
    if *pos + 2 > data.len() { return Err("读取方法注解映射计数越界".into()); }
    let entry_count = u16::from_le_bytes([data[*pos], data[*pos+1]]) as usize;
    *pos += 2;
    let mut map = HashMap::with_capacity(entry_count);
    for _ in 0..entry_count {
        if *pos + 4 > data.len() { return Err("读取方法ID越界".into()); }
        let method_id = u32::from_le_bytes([data[*pos], data[*pos+1], data[*pos+2], data[*pos+3]]) as usize;
        *pos += 4;
        if *pos + 2 > data.len() { return Err("读取注解计数越界".into()); }
        let ann_count = u16::from_le_bytes([data[*pos], data[*pos+1]]) as usize;
        *pos += 2;
        let mut annotations = Vec::with_capacity(ann_count);
        for _ in 0..ann_count {
            if *pos + 2 > data.len() { return Err("读取注解名长度越界".into()); }
            let nlen = u16::from_le_bytes([data[*pos], data[*pos+1]]) as usize;
            *pos += 2;
            if *pos + nlen > data.len() { return Err("读取注解名越界".into()); }
            let name = String::from_utf8_lossy(&data[*pos..*pos+nlen]).to_string();
            *pos += nlen;
            if *pos + 2 > data.len() { return Err("读取注解参数计数越界".into()); }
            let param_count = u16::from_le_bytes([data[*pos], data[*pos+1]]) as usize;
            *pos += 2;
            let mut parameters = Vec::with_capacity(param_count);
            for _ in 0..param_count {
                if *pos + 2 > data.len() { return Err("读取参数名长度越界".into()); }
                let plen = u16::from_le_bytes([data[*pos], data[*pos+1]]) as usize;
                *pos += 2;
                if *pos + plen > data.len() { return Err("读取参数名越界".into()); }
                let param_name = String::from_utf8_lossy(&data[*pos..*pos+plen]).to_string();
                *pos += plen;
                let value = deserialize_ann_value(data, pos)?;
                parameters.push((param_name, value));
            }
            annotations.push(MethodAnnotation { name, parameters });
        }
        map.insert(method_id, annotations);
    }
    Ok(map)
}

/// 反序列化注解值（S3）
fn deserialize_ann_value(data: &[u8], pos: &mut usize) -> BytecodeResult<AnnotationValue> {
    if *pos + 1 > data.len() { return Err("读取注解值类型标签越界".into()); }
    let tag = data[*pos]; *pos += 1;
    match tag {
        0 => {
            if *pos + 8 > data.len() { return Err("读取Int注解值越界".into()); }
            let v = i64::from_le_bytes([data[*pos],data[*pos+1],data[*pos+2],data[*pos+3],data[*pos+4],data[*pos+5],data[*pos+6],data[*pos+7]]);
            *pos += 8;
            Ok(AnnotationValue::Int(v))
        }
        1 => {
            if *pos + 8 > data.len() { return Err("读取Float注解值越界".into()); }
            let v = f64::from_le_bytes([data[*pos],data[*pos+1],data[*pos+2],data[*pos+3],data[*pos+4],data[*pos+5],data[*pos+6],data[*pos+7]]);
            *pos += 8;
            Ok(AnnotationValue::Float(v))
        }
        2 => {
            if *pos + 1 > data.len() { return Err("读取Bool注解值越界".into()); }
            let v = data[*pos] != 0; *pos += 1;
            Ok(AnnotationValue::Bool(v))
        }
        3 => {
            if *pos + 2 > data.len() { return Err("读取String注解值长度越界".into()); }
            let slen = u16::from_le_bytes([data[*pos], data[*pos+1]]) as usize;
            *pos += 2;
            if *pos + slen > data.len() { return Err("读取String注解值越界".into()); }
            let sv = String::from_utf8_lossy(&data[*pos..*pos+slen]).to_string();
            *pos += slen;
            Ok(AnnotationValue::String(sv))
        }
        4 => {
            if *pos + 4 > data.len() { return Err("读取Delegate注解值越界".into()); }
            let v = u32::from_le_bytes([data[*pos], data[*pos+1], data[*pos+2], data[*pos+3]]) as usize;
            *pos += 4;
            Ok(AnnotationValue::Delegate(v))
        }
        _ => Err("未知注解值类型".into()),
    }
}
...
mod tests {
    ...
    fn test_roundtrip_module() {
        let class = CompiledClass {
            ...
            method_override_id: vec![],
            field_start_counts: [0; 5],
            interface_method_impl_id: vec![],                    injector_constants: vec![],                    injector_constructor_impl_id: vec![],                    field_initializers: vec![],                    annotations: vec![],                    method_annotations: HashMap::new(),                    constructor_annotations: HashMap::new(),        };
        ...
        let class = CompiledClass {
            ...
            delegate_impls: vec![
                DelegateImpl {
                    ...
                    captured_var_types: vec![ValueType::Int],
                    outer_value_count: 1,
                    is_static: false,
                },
            ],
    ...
    /// V3 旧字节码兼容读取测试：不带注解段的字节流反序列化不报错、注解表空
    #[test]
    fn test_s3_v3_backward_compat_no_annotations() {
        // 构造一个不含方法注解的 V4 类，序列化后应正确反序列化
        let class = CompiledClass {
            class_type: GorgeType::class("V3Compat", None),
            is_native: false,
            super_class_name: None,
            super_interfaces: vec![],
            field_counts: TypeCount::zero(),
            methods: vec![CompiledMethod { name: "m".into(), codes: vec![], local_count: 0 }],
            constructors: vec![],
            injector_fields: vec![],
            delegate_impls: vec![],
            method_start_id: 0, method_count_total: 1, constructor_start_id: 0,
            method_override_id: vec![], field_start_counts: [0; 5],
            interface_method_impl_id: vec![], injector_constants: vec![],
            injector_constructor_impl_id: vec![], field_initializers: vec![],
            annotations: vec![],
            method_annotations: std::collections::HashMap::new(),
            constructor_annotations: std::collections::HashMap::new(),
        };
        let module = CompiledModule { version: 4, classes: vec![class] };
        let data = serialize_module(&module).unwrap();
        let deserialized = deserialize_module(&data).unwrap();
        assert_eq!(deserialized.classes.len(), 1);
        assert!(deserialized.classes[0].method_annotations.is_empty());
        assert!(deserialized.classes[0].constructor_annotations.is_empty());
    }

    /// 注解序列化→反序列化往返测试：注解信息完整保持
    #[test]
    fn test_s3_annotation_roundtrip() {
        use crate::objective::declaration::{MethodAnnotation, AnnotationValue};
        let mut ma = std::collections::HashMap::new();
        ma.insert(0, vec![MethodAnnotation {
            name: "ForwardTimedDestroy".into(),
            parameters: vec![
                ("time".into(), AnnotationValue::Float(2.5)),
                ("delegate_param".into(), AnnotationValue::Delegate(42)),
            ],
        }]);
        let mut ca = std::collections::HashMap::new();
        ca.insert(1, vec![MethodAnnotation {
            name: "InitializeGenerate".into(),
            parameters: vec![("priority".into(), AnnotationValue::Int(100))],
        }]);
        let class = CompiledClass {
            class_type: GorgeType::class("AnnRoundtrip", None), is_native: false,
            super_class_name: None, super_interfaces: vec![],
            field_counts: TypeCount::zero(),
            methods: vec![CompiledMethod { name: "m".into(), codes: vec![], local_count: 0 }],
            constructors: vec![],
            injector_fields: vec![], delegate_impls: vec![],
            method_start_id: 0, method_count_total: 1, constructor_start_id: 0,
            method_override_id: vec![], field_start_counts: [0; 5],
            interface_method_impl_id: vec![], injector_constants: vec![],
            injector_constructor_impl_id: vec![], field_initializers: vec![],
            annotations: vec![],
            method_annotations: ma, constructor_annotations: ca,
        };
        let module = CompiledModule { version: 4, classes: vec![class] };
        let data = serialize_module(&module).unwrap();
        let deserialized = deserialize_module(&data).unwrap();
        assert_eq!(deserialized.classes.len(), 1);
        let c = &deserialized.classes[0];
        // 方法注解
        let ma = c.method_annotations.get(&0).unwrap();
        assert_eq!(ma.len(), 1);
        assert_eq!(ma[0].name, "ForwardTimedDestroy");
        assert_eq!(ma[0].parameters.len(), 2);
        let time_val = ma[0].find_parameter("time").unwrap();
        assert!(matches!(time_val, AnnotationValue::Float(v) if (v - 2.5).abs() < 1e-9));
        let del_val = ma[0].find_parameter("delegate_param").unwrap();
        assert!(matches!(del_val, AnnotationValue::Delegate(42)));
        // 构造方法注解
        let ca = c.constructor_annotations.get(&1).unwrap();
        assert_eq!(ca.len(), 1);
        assert_eq!(ca[0].name, "InitializeGenerate");
        let pri = ca[0].find_parameter("priority").unwrap();
        assert!(matches!(pri, AnnotationValue::Int(100)));
    }
}
